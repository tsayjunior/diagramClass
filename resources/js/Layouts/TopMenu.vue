<template>
    <header class="z-10 py-4 bg-white shadow-md border-2 border-b-purple-800">
      <div class="container flex justify-between items-center px-6 mx-auto h-full text-purple-600 md:justify-end">
        <!-- Mobile hamburger -->
        <warningButton  @click="redirection()" v-if="show_navigation">
...
<script setup>
import { defineProps } from 'vue'; // Asegúrate de importar defineProps

// Definición del prop con tipo y valor predeterminado
const props = defineProps({
  show_navigation: {
    type: Boolean,
    default: false, // Puedes establecer un valor predeterminado si lo deseas
  },
});
...
const redirection = (sala)=>{
    // showModalView.value = true;
    
    Inertia.visit(route('salas.index'));
